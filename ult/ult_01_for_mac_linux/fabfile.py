...
def start():
    ...
    for key in data:
        ...
        port = data[key][1]
        usb = data[key][2]
        local( "sudo python process_worker.py --host=%s --port=%s --usb=%s &" % ( host, port, usb ) )
...
def stop():
    ...
    for eachline in fp:
        try:
            local( "sudo kill %d" % int(eachline) )
        except:
            print red( "aready" )
    local( "sudo cat /dev/null > pid_repo" )
